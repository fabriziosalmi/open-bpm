...
For each of the 664 tracks, the following metrics were computed at four BPM points: ground truth, detected, half of detected, and double of detected. The validation tool ([`src/bin/validate_metrics.rs`](https://github.com/fabriziosalmi/open-bpm/blob/main/src/bin/validate_metrics.rs)) outputs a TSV with all values; analysis was done with awk.
